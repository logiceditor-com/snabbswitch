...
assert(isLittleEndian(), "big-endian platform not supported")
...
-- based on make_concatter from https://github.com/lua-nucleo/lua-nucleo
-- MIT licensed
local make_concatter = function()
 ...
 local INDENT_SIZE = 3
   ...
   local function undent ()
      indent_level = indent_level == 0 and 0 or indent_level - 1
   end
   local methods = { undent = undent, indent = indent }
 ...
 local function cat(...)
   ...
   buf[#buf + 1] =
      string.rep(" ", INDENT_SIZE * indent_level) .. table.concat(t)
   return cat
 end
...
function selftest ()
   ...
   app.apps.source1 =
      app.new(pcap.PcapReader:new("src/apps/packet_filter/v6.pcap"))
   ...
   app.apps.source2 = app.new(pcap.PcapReader:new("src/apps/packet_filter/v4.pcap"))
   app.apps.packet_filter2   = app.new(PacketFilter:new({v4_rule_dns, v4_rule_tcp}))
